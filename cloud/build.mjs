...
for(const name of ['index.html','script.js','style.css','data.js','campaign.js','campaign-ui.js','core.js','layers.js']){let text=await fs.readFile(name,'utf8');text=text.replaceAll('.png','.webp');await fs.writeFile('dist/client/'+name,text);}
let count=0,bytes=0;
for(const dir of ['scenes','cutouts-complete','layers']){await fs.mkdir('dist/client/assets/'+dir,{recursive:true});for(const name of await fs.readdir('assets/'+dir)){if(!name.endsWith('.png'))continue;const out='dist/client/assets/'+dir+'/'+name.replace('.png','.webp');await sharp('assets/'+dir+'/'+name).webp({quality:88,alphaQuality:100}).toFile(out);bytes+=(await fs.stat(out)).size;count++;}}
