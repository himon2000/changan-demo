...
function play(choice=0,pass=true){
 ...
 r.shared.campaign.chapter='qujiang';r.games.A.enter('introA');r.games.B.enter('introB');
 ...
 assert.equal(r.games.A.s.view,'deduce');for(const role of ['A','B'])for(const [key,value]of Object.entries({suspect:'host',method:'mechanism',motive:'poem'}))cmd(role,'deduce',{key,value});cmd('A','submit');assert.equal(r.games.A.s.view,'deduce');cmd('B','submit');assert.equal(r.games.A.s.view,'end');assert.equal(r.games.B.s.view,'end');assert.equal(r.shared.flags.correct,true);assert.equal(r.shared.flags.chaseSuccess,pass);assert.equal(r.shared.flags.saved,choice===0);
...
test('all network page states render, with only owned evidence choices and no role switch',()=>{
 ...
 const context={console,Game,GAME_DATA:D,URLSearchParams,location:{protocol:'file:',search:'',origin:'http://localhost:8787'},performance:{now:()=>0},Image:class{},sessionStorage:{getItem(){return null},setItem(){},removeItem(){}},requestAnimationFrame:()=>0,cancelAnimationFrame(){},setTimeout:()=>0,clearTimeout(){},document:{getElementById:id=>id==='app'?app:null,querySelector:()=>null,addEventListener(){},hidden:false},addEventListener(){}};context.window=context;vm.createContext(context);for(const file of ['layers.js','campaign.js','campaign-ui.js'])vm.runInContext(fs.readFileSync(path.join(__dirname,'../'+file),'utf8'),context);
