...
const P=root.CAMPAIGN||(typeof require==='function'?require('./campaign.js'):null);
...
class Game{
 ...
 sceneFor(e){if(e==='prologueA')return 'temple';if(e==='prologueB')return 'academy';if(['meeting','mirrorChoice','mirrorAfter'].includes(e))return 'market';if(e==='qujiangReflection')return 'lake';if(P.events[e])return P.current(this.s).scene;return {introA:'temple',introB:'academy',feast:'boat',witness:'shrine',workshop:'academy',reunion:'boat',waterEntry:'water',alarm:'water',afterWater:'water',beforeChase:'dock',afterChase:'dock',revisit:'boat'}[e]||(e.startsWith('side:')?e.slice(5):'boat');}
 lines(){const s=this.s,f=s.flags,a=s.role==='A';const L=line,C=choice;
 if(P.events[s.event])return P.events[s.event](s,L,C);
 switch(s.event){
 ...
 case 'feast':return [L('n',a?'巡捕把腰牌收进衣袖，随客人登上画舫。':'幻术师沿船舷走了一圈。灯座朝向被人改过。'),L('host','请坐。今夜只论诗，旁的事，改日再说。'),L('poet','有些事，过了今夜就说不清了。','write'),L('n','主持笑着转身。诗人独自走进侧舱。片刻后，灯火暗了一息。'),L('boatwoman','舱门怎么扣上了？公子？','point'),L('n','门被撞开。诗人伏在矮桌前，外袍干燥，面前的银杯倒着。指甲缝里却有湿泥。'),L('A','脉息没了。谁也别碰桌子。','examine'),L('B','这扇门可以从外面扣上。我需要看看灯座。','examine'),L('A','我去问人，你留下查船。', 'idle',[C('把机关调查交给幻术师',{stats:{TA:5,TB:3},flags:{delegated:true}},'幻术师可独立拆验灯座'),C('先封存机关，要求每步留痕',{stats:{TB:-3},flags:{delegated:false}},'幻术师需要先取得结构证据')]),L('n','两人分开。一人追问岸上的时间，一人追索船里的光线。')];
 ...
 case 'workshop':return [L('n','幻术师独自回到学院。旧图上，画舫的席板下藏着一层水舱。'),L('B','席板能把人送进下层。这里还画着检修梯与回升的配重。','examine'),L('n','灯座后还有一条牵线孔，正好通到门闩。'),L('B','这段结构，得让巡捕知道。','examine',[
 ...
 books:[L('n','账本夹在新诗集里。画舫早被抵押，船工的工钱却始终没有入账。'),L(a?'A':'B',a?'账页是掩饰。夹层那首写着废宫的诗，才是他准备在今夜公开的东西。':'新纸压着旧字。有人想改日期，终究没敢撕掉签名。','examine')],
 ...
 submit(){const s=this.s,d=s.deduction;if(!d.suspect||!d.method||!d.motive)return false;if(d.method==='mechanism'&&!this.precise())return false;s.flags.correct=d.suspect==='host'&&d.method==='mechanism'&&d.motive==='poem'&&s.evidence.includes('ledger');s.flags.partial=d.suspect==='host'&&!s.flags.correct;this.apply({stats:s.flags.correct?{TRUTH:3}:s.flags.partial?{CHAOS:2}:{CHAOS:5,DREAM:3}});s.view='end';s.phase='end';return true;}
 task(){const s=this.s;if(s.campaign?.chapter!=='qujiang'){const c=s.campaign,ch=P.current(s);if(c.chapter==='prologue'&&c.stage==='intro')return '到东市追捕抱镜的工匠，与另一位追踪者会合。';if(c.chapter==='final')return '继续向下。';return ch.title+' · '+(c.found[s.role].length<2?'调查：'+ch[s.role]:!c.witness[s.role]?'前往问询地点，补足自己的记录。':'记录已备。可继续查隐藏线索，或与搭档会合。');}if(!s.flags.murder)return '赴湖中画舫，出席今夜诗会。';if(!s.flags['cabin'+s.role])return s.role==='A'?'回画舫查验杯盏、诗笺与地上的水痕。':'回画舫检查门闩，寻找从舱外牵动机关的方式。';if(s.role==='A'&&!s.flags.witness)return '去岸边祠亭，听船娘说完她的经过。';if(s.role==='B'&&!s.flags.workshop)return '回幻术院核对旧图，再决定向搭档分享什么。';if(!this.readyToMeet())return '你的调查已告一段落。等待搭档完成调查，也可以继续探访街巷。';if(!s.flags.validated)return '双方记录已经齐备。回画舫，将两份发现相互验证。';if(!s.flags.waterDone)return '回画舫，下到水舱查验铜轮与止水绳。';return '将现有证据串起，给出案件判断。';}
}
