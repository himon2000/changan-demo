...
const choice=(text,effects={},hint='')=>({text,effects,hint});
class Game{
 ...
 setRole(role){this.s.role=role;this.s.lead=role;this.enter('intro'+role);}
 ...
 C('把结构图交给巡捕',{stats:{TA:5},flags:{sharedMechanism:true}},'巡捕获得换灯追问与配重救援选项'),C('先自己留着，等看过实物再说',{stats:{TA:-3},flags:{sharedMechanism:false}},'巡捕暂时只能从门外救援'),]),...(f.delegated&&s.stats.TA>=55?[L('B','他肯把现场交给我，我也该把判断交给他。','examine',[C('另附灯座牵线的验证方法',{stats:{TA:3},flags:{mirrorSignal:true}},'巡捕可在追逐中辨认反光暗号'),C('只发送结构图',{})])]:[L('B','封条还在。先按旧图找入口，别毁掉门闩上的痕迹。')])];
 ...
 ...(f.sharedMechanism?[C('踩住配重，给搭档留出转轮时间',{stats:{TB:5},flags:{support:'weight'}},'幻术师使用转轮、卡齿、排水动作')]:[]),C('顶住舱门，让搭档拉住止水绳',{stats:{CHAOS:2},flags:{support:'door'}},'幻术师使用绳索、撑门、攀梯动作')])];
 ...
 case 'beforeChase':return [L('n',f.saved?'巡捕伸手将搭档拉上甲板。两人踏着湿木板赶向岸边。':'幻术师放开上方绳索，扳开石渠。退水时，一段陌生城墙的倒影划过脚边。'),L('B',f.mirrorSignal?'看水边的反光，我会替你指出落脚处。':'右边的桥板松了。别踩中央。','action'),L('A','这回，换我跟着你的判断。','action')];
 ...
 advance(i=0){if(this.s.view!=='dialog')return false;const l=this.currentLine();if(l.choices){const c=l.choices[i];if(!c)return false;this.apply(c.effects);}this.s.index++;if(this.s.index>=this.lines().length)this.finishEvent();return true;}
 ...
 switchRole(){const s=this.s;if(!['map','investigate'].includes(s.view))return false;s.role=s.role==='A'?'B':'A';if(s.view==='map'&&!s.flags['intro'+s.role])this.enter('intro'+s.role);return true;}
 ...
 qteSteps(){const s=this.s;if(s.qte.type==='water')return s.flags.support==='weight'?[['逆转铜轮','A','转轮向左'],['卡住齿口','D','把木楔推入'],['打开泄水闸','W','提起闸杆']]:[['攥紧止水绳','A','稳住绳索'],['顶开落下的舱门','D','撑住门缘'],['抓住上方横梁','W','向上攀住']];return s.lead==='A'?[['跃过松动桥板','W','向前跃起'],['避开迎面船篙','A','向左闪身'],['截住深色披风','D','扣住手腕']]:[['将月光引向安全的桥板','A','偏转铜镜'],['抛出绳索封住去路','D','向右投绳'],['压住配重，为搭档落脚','W','稳住踏板']];}
